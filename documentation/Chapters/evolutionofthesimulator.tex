\chapter{Evolution of the simulator}
	...
	\section{Model behavior verification}
		In Section \ref{sec:base2car} two examples were shown where the model produced the same stationary state for different initial conditions. Stationary state means in this case that the vehicle's acceleration is zero. So equation \ref{eq:aidm} modifies as followings in a stationary state:
		\begin{equation}
	...
	\section{Solver for n cars}
		...
		\end{figure}
		Unfortunately the previously used implementation is not good enough to produce $\dot{\textbf{y}}$. The problem is  that to be able to calculate vehicle n's position and velocity the solver needs to have vehicle n-1's position and velocity. To make it easy to read let us use function $f$ which represents equation \ref{eq:numerical_idm}. Than $\dot{\textbf{y}}$ looks like the following:
		\begin{equation}
			\dot{\textbf{y}}=
			\begin{pmatrix}
				100\\
