\chapter{Analysis}
	...
	\section{Self driving car variations}
		...
		As seen in the previous examples the more self-driving vehicles in the traffic the better. So let us investigate all possible combinations to see that is the connection between the time and the number of self-driving cars. The expectation is that the after a certain point self driving car cannot make traffic better. Until that point the connection is linear.
		So all the possible combinations were built from only one exchanged car until all the ten cars exchanged. The total number of simulations were 1023. The plot of 
		\begin{figure}[ht]
			\centering
