\chapter{Final model}
	...
	\section{Solution for traffic light situation}
		...
		\end{equation}
		The current acceleration value is represented by $|a|$ and a threshold value $a_{\rm tr}$ is between 0 and $\amax$ for a human driver.
		\subsection*{Improvement III}
		Until this point the simulator was running until a specific time point. It would be nice to run until when all of the cars have reached a specific distance. For example a traffic light situation is interesting for the first 100 meters.