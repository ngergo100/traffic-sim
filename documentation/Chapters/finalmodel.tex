\chapter{Final model}
	...
	\section{Solution of traffic light situation}
	...
	\section{Solution with improvements}
		\subsection*{Initial conditions}
		The initial conditions remained the same except additional parameters are necessary to provide for the improvements. The initial conditions with the additional parameters can be found in Table \ref{tab:new_array}. the new initial conditions will contain the time intervals whenever a driver is not paying attention as well as the acceleration threshold value.
		\begin{table}
			\begin{center}
				\begin{tabular}{ |c|c|c|c| }
					\hline
					Id & ... & Not paying attention & $a_{\rm p,tr}$\\
					$[-]$ & & [s]& $\rm [m/s^2]$\\
					\hline
					1 & ... & 24-26 & 1.2 \\
					2 & ... & 0-2 & 1.2 \\
		...
		\end{figure}
		\subsection*{Summary of the result}
	\section{Behavior of a self driving vehicle}
