\chapter{Final model}
	...
	\section{Simulation of a traffic light situation}
		...
		Results of the simulation can be seen on Figure~\ref{fig:red_light_situationh_impr3}, \ref{fig:red_light_situationv_impr3} and \ref{fig:red_light_situationcc_impr3}. After implementing this improvement the overall duration has increased to \casetime{4}seconds. With this improvement the headway and velocity figures have changed significantly. The time intervals when a driver is not paying attention to driving can be seen in Figure~\ref{fig:red_light_situationv_impr3}. The horizontal parts of a curve (where $\dot{v}=0$) show that a driver is probably distracted from driving.
		\begin{figure}
			\centering
