\documentclass[a4paper,12pt,twoside, openright]{report}

% Packages
\usepackage{amsmath} % -> Matrices, `pmatrix` command
\usepackage[toc,page]{appendix} % -> Include appendix
\usepackage{auto-pst-pdf} % -> Dependency of psfrag. Add `-shell-escape` paramter to pdflatex command in Commands menu
\usepackage[top=2cm, bottom=2.5cm, inner=2cm, outer=2cm, bindingoffset=1.5cm]{geometry} % -> Makes custom margins for the twoside document style. Use `\usepackage{showframe}` command to show the margins
\usepackage{graphicx} % -> Images, `includegraphics` command
\usepackage[hidelinks]{hyperref} % -> Using clickable urls
\usepackage[parfill]{parskip} % -> Makes new paragraphs without indentation.
\usepackage[final]{pdfpages} % -> Include pdf pages
\usepackage{psfrag} % -> To be able to change text font on matlab exports, `psfrag` command


% Other configuration
\graphicspath{{./Resources/}}
\input{epstextreplacer}

% Custom commands
\newcommand{\amax}{a_{\rm max}}
\newcommand{\bmax}{b_{\rm max}}
\newcommand{\vd}{ {v_{\rm d}}}
\newcommand{\afree}{a_{\rm free}}
\newcommand{\vt}{v(t)}
\newcommand{\afollower}{a_{\rm follower}}
\newcommand{\hd}{h_{\rm d}(t)}
\newcommand{\h}{h(t)}
\newcommand{\posleader}{x_{\rm lead}(t)}
\newcommand{\lengthleader}{L_{\rm lead}}
\newcommand{\vleader}{v_{lead}(t)}
\newcommand{\id}{{\rm id}}
\newcommand{\vcl}{l_{\rm current}}
\newcommand{\vtl}{l_{\rm target}}
\newcommand{\casetime}[1]{\input{Resources/eemobil/simt_case#1}}
\newcommand{\vehicledensitytable}[1]{\input{Resources/eemobil/vehicle_density_table_case_#1}}
\newcommand{\vehicledensitypartable}[1]{\input{Resources/eemobil/vehicle_density_par_pos_tab_case#1}}

\begin{document}
	\input{titlepage}
	\input{copyright}
	\includepdf[pages=-]{task.pdf}
	\input{declaration}
	\pagenumbering{gobble}
	\tableofcontents
	\input{Chapters/introduction}
	\pagenumbering{arabic}
	\input{Chapters/trafficmodels}
	\input{Chapters/evolutionofthesimulator}
	\input{Chapters/finalmodel}
	\input{Chapters/cases}
	\input{Chapters/analysis}
	\input{Chapters/summary}
	\input{Chapters/osszefoglalas}
	\bibliographystyle{unsrt}
	\bibliography{documentation}
	\input{Chapters/appendix}
\end{document}
